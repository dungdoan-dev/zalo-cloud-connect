...
function dialplanXml(config) {
  ...
  const outbound = config.accounts.map((account) => {
    ...
    const originate = `{origination_caller_id_number=${xml(account.oaId)},origination_caller_id_name=${xml(account.name)},sip_from_user=${xml(account.oaId)},sip_from_host=${xml(domain)},sip_from_uri=sip:${xml(account.oaId)}@${xml(domain)}}sofia/zcc/$1@${xml(domain)};transport=tcp`;
