...
function ui() { return Object.fromEntries(['float','float-toggle','float-status','float-message','target-type','callee','call','answer','decline','mute','hold','remote-audio'].map((name) => [`${name.replace(/-([a-z])/g, (_, c) => c.toUpperCase())}`, document.getElementById(`zcc-${name}`)])); }
